...
# Hint, use "time" and "status" as the time variable. Treatment is trt

# Fit Cox proportional hazards model
model_c <- coxph(Surv(..., ...) ~ ...., data = veteran)
summary(...)
...
ap_surv <- avg_predictions(model_c,
                           type = ..., # survival scale
                           by = ..., # Put time variable and treatment here
                           vcov = ..., # non-parametric bootstraping
                           newdata = ...) # The new dataset we just simulated
...
time_comp <- comparisons(
  ..., # Put the model we just estimated
  variables = ..., # treatment variable
  newdata = ..., # Specify time with 100, 200, 300, 400, 500
  type = ... # survival scale
)
